...
#    | |/ _ \| |/ / | | | |    / _` | '_ \/ __|
#    | | (_) |   <| |_| | |___| (_| | |_) \__ \
#    |_|\___/|_|\_\\__, |______\__,_|_.__/|___/
#                   __/ |
#                  |___/
...
class Application(tk.Frame):
  def __init__(self, master=None):
    ...
    self.master.title('Tokymaker Firmware Updater')
    self.master.grid_columnconfigure(0,minsize=300,weight=1)
    self.grid()
    self.createWidgets()
  ...
  def downloadLatestFirmware(self):
    ...
    self.downloadFile(version, "TokyMaker.bin")
    self.download_text.set("...Latest Firmware Downloaded!")
  ...
  def createWidgets(self):
    ...
    self.download_target = tk.IntVar()
    self.can_execute = tk.StringVar()
    ...
    self.versionBetaButton = tk.Radiobutton(self, text='Release', variable=self.firmware_type, value=1)
    self.versionBetaButton.grid(row=0,column=0,sticky='WE')
    self.versionReleaseButton = tk.Radiobutton(self, text='Beta', variable=self.firmware_type, value=2)
    self.versionReleaseButton.grid(row=0,column=1,sticky='WE')

    self.downloadLatestButton = tk.Button(self, text='Download Latest Firmware', command = self.downloadLatestFirmware)
    self.downloadLatestButton.grid(row=1,column=0,columnspan=2,sticky='WE')

    self.downloadTextLabel = tk.Label(self, textvariable=self.download_text)
    self.downloadTextLabel.grid(row=2,column=0,columnspan=2,sticky='WE')

    self.downloadProgress = ttk.Progressbar(self, orient='horizontal', length = 200, variable = self.download_progress, maximum = 100, mode='determinate')
    self.downloadProgress.grid(row=3,column=0,columnspan=2,sticky=tk.E+tk.W)

    #self.specifyFirmwareButton = tk.Button(self, text='Choose Firmware (Optional)', state='normal', command=self.openFirmwareFile)
    #self.specifyFirmwareButton.grid(row=4,column=0)
    #self.firmwarePathLabel = tk.Label(self, textvariable=self.firmware_path)
    #self.firmwarePathLabel.grid(row=4,column=1)
    ...
    if self.check_can_update():
      self.executeButton.configure(state = 'normal')
    self.executeButton.grid(row=6,column=0,columnspan=2,sticky='WE')
    self.flashwareStatus = tk.Label(self, textvariable=self.firmware_status)
    self.flashwareStatus.grid(row=7,column=0,columnspan=2,rowspan=10,sticky='WE')
